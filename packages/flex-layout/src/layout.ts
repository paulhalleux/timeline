...
export type FlexToolbarSide = "left" | "right" | "top" | "bottom";
...
export interface FlexDropLocation {
  ...
  /** Insertion index used when rearranging panels inside a center tabset drop. */
  readonly index?: number;
}
        ...
        node: {
          ...
          panels:
            location.index === undefined
              ? insertAfter(node.panels, location.targetPanelId, panelId)
              : insertAt(node.panels.filter((id) => id !== panelId), panelId, location.index),
...
function addToolbarPanel(
  ...
  if (
    getToolbarPanelIds({ root: null, panels: {}, hiddenPanelIds: [], toolbars }).includes(
      panelId,
    )
  ) {
    return toolbars;
...
function removeToolbarPanel(
  ...
  return Object.fromEntries(
    flexToolbarCorners.map((corner) => [
      corner,
      removePanelFromToolbarGroups(toolbars[corner], panelId),
    ]),
...
function insertPanelIntoToolbarGroups(
  ...
  const index = Math.max(
    0,
    Math.min(location.index ?? targetGroup.panelIds.length, targetGroup.panelIds.length),
  );
  const nextGroup = {
    ...targetGroup,
    panelIds: [
...
function insertAt<T>(items: readonly T[], item: T, index: number): readonly T[] {
  const next = [...items];
