...
const TrackRoot = React.memo(({ children, height, style, className, ...rest }: TrackRootProps) => {
  return (
    <div
...
const TrackHeader = React.memo(({ children, style, className, ...rest }: TrackHeaderProps) => {
  const width = useTimelineStore((timeline) => timeline.getViewport().getHeaderOffsetPx());
...
const TrackContent = React.memo(({ children, className, style, ...rest }: TrackContentProps) => {
