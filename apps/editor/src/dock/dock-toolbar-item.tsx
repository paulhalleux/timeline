...
export function ToolbarItemButton({ item, side }: ToolbarItemButtonProps) {
  ...
  const active = currentPlacement ? state.placements[currentPlacement].activeItemId === item.id : false;
    ...
    <ContextMenu>
      ...
      <ContextMenuContent side={side === "left" ? "right" : "left"} align="start">
        ...
        {currentPlacement ? (
          <>
            <ContextMenuSeparator />
            <ContextMenuItem onClick={() => dock.floatToolWindow(item.id)}>
              Float
            </ContextMenuItem>
            <ContextMenuItem
              variant="destructive"
              onClick={() => dock.hideToolWindow(item.id)}
            >
              Hide
            </ContextMenuItem>
          </>
